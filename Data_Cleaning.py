...
df = pd.read_json('Project_Datasets/GCO_Dataset.json')
df.to_csv('Project_Datasets/WHO_Data/GCO_Dataset.csv', index=False)
...
projected.rename(columns = {'year': 'Year','id_label':'Country','cancer_label': 'Cancer_Type','cases_pred': "Predicted_Cases",'pop': 'Population','change':'Predicted_Increase',
                            'percent':'Predicted_Percentage_Change'}).to_csv('Cleaned_data/GCO_Pred_Data.csv', index = False)

WHO_data = pd.read_csv('Project_Datasets/WHO_Data/WHOMortalityDatabase_Map_Melanoma and other skin cancers_19th December 2023 23 15.csv', skiprows = 6, index_col=False)
WHO_data = WHO_data.drop(columns = ['Region Code','Region Name','Country Code','Age group code']).fillna(0).replace("T?rkiye", "Turkey")
...
grim_df = GRIM_data.drop(columns = ['grim']).fillna(0).replace(["Missing", "Total", "Skin cancer (ICD-10 C43, C44)"], ["Unknown", "All", "Skin Cancer"])
...
Cancer_data = pd.read_excel('Project_Datasets/AIHW_Data/aihw-can-122-cancer-data-commentaries.xlsx', sheet_name = 'Table S4', skiprows = 2, nrows = 79)
Cancer_data = Cancer_data.drop(columns = ['Age range (years)','Unnamed: 5','Unnamed: 8', 'ICD10 codes', 'Unnamed: 10']).rename(columns = {'Background data:\nCumulative number of\n people diagnosed': 'Cumulative number of people diagnosed',
