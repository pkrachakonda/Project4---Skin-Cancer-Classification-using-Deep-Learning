...
test_df['image_path'] = glob('Project_Datasets//ISIC_2019_Test_Input//*.jpg')
...
# for training data
train_file_dir = 'Project_Datasets/ISIC - 2019/train'
image_size = (180, 180)
...
# for testing data
test_file_dir = 'Project_Datasets/ISIC - 2019/test'
image_size = (180, 180)
...
X_test = []
y_test = []
for i, j in test_ds:  # i= images , j=labels of images
    X_test.append(i)
    y_test.append(int(j))
...
### Saving training and test dataset values

np.save('Cleaned_Data/X_train_v01', X_train)
np.save('Cleaned_Data/y_train_v01', y_train)
np.save('Cleaned_Data/X_test_v01', X_test)
np.save('Cleaned_Data/y_test_v01', y_test)
...
def test_model(model, X_test, Y_test):
    ...
    print(clr)
    pd.DataFrame(clr).transpose().to_csv('model/Classification_Report_v01.csv', index = True)
    ...
    for i in range(15):
        ...
        plt.axis("off")
    plt.show()
    plt.savefig('images/Comparison of Observation vs Model Prediction_v01.jpg')
...
callback = tf.keras.callbacks.ModelCheckpoint(filepath='model/best_model.h5',
...
plt.savefig('images/Fig10_Training_History_v01.jpg')
...
plt.show()
plt.savefig('images/Confusion Matrix_v01.jpg')
...
with open('training_history_v01.pkl', 'wb') as file:
    pickle.dump(training_history, file)
model.save('model/EDA_CNN_Trained_Model_v01.h5')
model.save('model/EDA_CNN_Trained_Model_v01.keras')
...
model=load_model('model/EDA_CNN_Trained_Model_v01.h5')
