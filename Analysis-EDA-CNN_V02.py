...
CNN_df['image_pixel'] = CNN_df['image_path'].progress_map(
    lambda x: np.asarray(Image.open(x).resize((180, 180))))
...
print(X_aug.shape)
print(Y_aug.shape)
np.save('Cleaned_Data/X_aug', X_aug)
np.save('Cleaned_Data/Y_aug', Y_aug)

X_aug = np.load('Cleaned_Data/X_aug.npy')
Y_aug = np.load('Cleaned_Data/Y_aug.npy', allow_pickle=True)
...
train_generator = datagen.flow(X_aug, Y_aug, batch_size=128, shuffle=True, seed=133, ignore_class_split=True,
                               subset="training")

validation_generator = datagen.flow(X_aug, Y_aug, batch_size=128, shuffle=True, seed=133, ignore_class_split=True,
                                    subset="validation")

X_train = train_generator.x
X_test = validation_generator.x

y_train = train_generator.y
y_test = validation_generator.y

### Saving training and test values

np.save('Cleaned_Data/X_train_v02', X_train)
np.save('Cleaned_Data/y_train_v02', y_train)
np.save('Cleaned_Data/X_test_v02', X_test)
np.save('Cleaned_Data/y_test_v02', y_test)
...
def test_model(model, X_test, Y_test):
    ...
    print(clr)
    pd.DataFrame(clr).transpose().to_csv('model/Classification_Report_v02.csv', index = True)
    ...
    for i in range(15):
        ...
        plt.axis("off")
    plt.show()
    plt.savefig('images/Comparison of Observation vs Model Prediction_v02.jpg')
...
plt.savefig('images/Fig10_Training_History_v02.jpg')
...
with open('model/training_history_v02.pkl', 'wb') as file:
    pickle.dump(training_history, file)
model.save('model/EDA_CNN_Trained_Model_v02.h5')
model.save('model/EDA_CNN_Trained_Model_v02.keras')
...
model=load_model('model/EDA_CNN_Trained_Model_v02.h5')

import pickle
with open('model/training_history_v02.pkl', 'rb') as f:
